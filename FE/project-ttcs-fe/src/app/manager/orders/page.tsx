...
export default function ManagerOrdersPage() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      const response = await orderApi.getAllManager({
        page,
        size: PAGE_SIZE,
  ...
  const handleUpdateStatus = async (orderId: number, status: OrderStatus) => {
    ...
    try {
      await orderApi.updateManagerStatus(orderId, status);
      await fetchOrders();
    } catch (err) {
      const apiError = err as ApiError;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">
            Chỉ hiển thị và cập nhật đơn hàng thuộc chi nhánh của tài khoản manager.
          </p>
        </div>
                ...
                Array.from({ length: 5 }).map((_, index) => (
                  <tr key={index} className="animate-pulse">
                    <td className="px-8 py-6">
                      <div className="h-12 bg-slate-100 rounded-2xl w-44" />
                    </td>
                  ...
                  <tr key={order.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-8 py-6">
                      <select
                        ...
                        onChange={(event) =>
                          order.id && handleUpdateStatus(order.id, event.target.value as OrderStatus)
                        }
